...
import { ShoppingCart, Search, Menu, X, Heart, UserRound } from "lucide-react";
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-8 h-16 sm:h-20 flex items-center justify-between gap-4">
        ...
        >
          <img
            src={logo}
            alt="OmniCart"
            className="h-11 sm:h-14 w-auto max-w-40 object-contain"
          />
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3 flex-1 max-w-md justify-end">
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 grid place-items-center min-w-4.5 h-4.5 px-1 text-[11px] font-bold text-white bg-brand-600 rounded-full ring-2 ring-white">
                {cartCount}
              </span>
            )}
          </Link>
            ...
            {navLinks.map((link) => (
              ...
              >
                ...
                {link.label === "Cart" && cartCount > 0 && (
                  <span className="grid place-items-center min-w-5 h-5 px-1 text-[11px] font-bold text-white bg-brand-600 rounded-full">
                    {cartCount}
                  </span>
                )}
              </NavLink>

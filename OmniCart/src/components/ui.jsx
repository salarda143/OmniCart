	...
	return (
		<div className="bg-white rounded-2xl border border-slate-100 shadow-card p-5 hover:shadow-soft transition">
			<div className="flex items-start justify-between">
				<div
					className={`grid place-items-center w-11 h-11 rounded-xl bg-gradient-to-br ${boxTones[tone]} text-white shadow-md`}
				>
...
export function Card({ title, action, children, className = "" }) {
	return (
		<div className={`bg-white rounded-2xl border border-slate-100 shadow-card ${className}`}>
			{(title || action) && (
